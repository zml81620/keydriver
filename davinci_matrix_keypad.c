...
static int decode_row_status(unsigned long data,const unsigned char new_status,\
           const unsigned char old_status,const int row_index,const int col_index)
{
    ...
    if(flage)
    {
       ...
       row2_status = new_status;
    }
    return flage;
}

static int real_process_row2(unsigned long data,const unsigned char status)
{
    int col = -1;
    int flage = 0;
    ...
    if(col >= 0)
    {
       flage = decode_row_status(data,status,row2_status,2,col);
    }
    return flage;
}
...
static int matrix_kp_process_row2(unsigned long data)
{
    ...
    if(changed)
    {
        ...
        if(changed)
        {
           printk(KERN_ERR "\n new_status=%x,old_status=%x\n",new_status,row2_status);
           flage = real_process_row2(data,new_status);
        }
    }
    return flage;
}
...
static void matrix_kp_scan(unsigned long data)
{
        ...
        flage = matrix_kp_process_row2(data);
        if(!flage)
        {
          //´¦Àírow0 row1
        }
